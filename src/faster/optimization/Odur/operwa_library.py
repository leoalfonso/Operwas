...
from operwas.operwa_inputs import *
from operwas.user_input_import import *
from operwas.operwa_TT_module import runIntegration, runTreatmentModuleIntegration,toV1CostArray,normalizeV2ZeroFlows
logging.basicConfig()
gdal.UseExceptions()
...
#todo GER: the function below was used to calc land cost in V1
...
def calculate_applied_irr_flow(radius_wwtp, inputfn, feature_grid_data, wwtp_index):
    ...
    # TODO: write comment: This is where to select a train from the v2 key-value store of trains: TO NOTE if incoorporated a
    # Need to modify this root function (calculate_applied_irr_flow) to take in q_inf to help
    # compute part_2 of the key to use in selecting a train
    # choice of if will constiture part_1 of the key use in selecting a train
...
#TODO MD 4: ACTION- Write comment. This is the function that determines the treatment technology in V1,
...
def get_feature_grid(path_data, tmp_filename):
    ...
    # index.close()
    return index
    #TODO M4 : ACTION- Write comment for this function
def onsite_treat_costs(total_pop, cluster_pop):
    '''
    This function estimates cost for serving ...........
...
def Join_Calcul(num_WWTP, coordinates):
    ...
    #This calculaTes wastewater generation in each sub-catchment##
    ...
    #TODO M4: ACTION- Modification done. Write comment
    '''
    Gerald modification, to trigger cost calculations in V2 
    calc_cost(flow_wwtp=flow_wwtp, method=2) 
    '''
    ...
    # TODO M4: ACTION, Here the type of treatment printed is the V1s , how to get V2 here? if V2
    ...
    # TODO M4: ACTION- MODIFICATION DONE. Write comment on  how V1 & V2 will operate in cost cal
    '''
        If the method is from version 2, 
    The treatment_costs is obtained from the treatment module in version 2,
    Based on technology selection criteria for a given reuse and flow/person equivalent
        If the method is the original "Maria's"version 1, 
    The cost calculation continues as per version 1 method,
    Based on CAS for agricultural irrigation use, and MBR for urban irrigation or No-reuse
    '''
    ...
    '''
    If method is from version 2, the treatment cost 
    '''
    treatment_costs = []
    treatment_train_data_v2 = []

    if VERSION == VERSION_TWO:
        treatment_train_data_v2 = runTreatmentModuleIntegration(
            flow_wwtp, types_treatment_flow_based
        )

    else: # keep running Maria's version of costs calculation
        for i in range(len(types_treatment_flow_based)):
            if types_treatment_flow_based[i] == "no_wwtp":
                ...
                treatment_costs.append(
                    wwtp_costs(
                        ...
                        known_flow_cas_1,
                        known_flow_cas_2,
                        known_inv_cost_cas_1,
                        known_inv_cost_cas_2,
                        known_oem_cost_cas_1,
                        known_oem_cost_cas_2,
                        DR,
                        n,
                    )
                )
                ...
                treatment_costs.append(
                    wwtp_costs(
                        ...
                        known_flow_mbr_1,
                        known_flow_mbr_2,
                        known_inv_cost_mbr_1,
                        known_inv_cost_mbr_2,
                        known_oem_cost_mbr_1,
                        known_oem_cost_mbr_2,
                        DR,
                        n,
                    )
                )

    #TODO MD 4: Write comment

    if VERSION == VERSION_TWO:
        treatment_costs = toV1CostArray(treatment_train_data_v2, ["capital_cost", "operational_cost", "energy_cost"])
        print('Treatment costs (capital+operation+energy) with varying TTs  = ', treatment_costs, 'a total = ', sum(treatment_costs), 'ILS')
    else:
        print('Treatment costs with wwtps varying between MBR and CAS (investment and OeM) = ', treatment_costs,
              'a total = ', sum(
                treatment_costs), 'ILS')

    # Costs of land purchase
    # TODO MD 3:ACTION-  this function below helps get the unit land cost in each wwtp_location(ie, @ Qinf)
    #  # So, where Unit land cost "from knowledge base" in V2 lib (about line 354),Refer to 319
    #  #r will be replaced with land_cost_in_wwtp_location to calculate land cost

    # TODO MD 4: ACTION- provide comment- here land_cost_by_wwtp calls a func get_land_cost_by_wwtp from Method/Version 1,
    #  and also calculates the sum of the obtained land costs
    #  land cost from V2 adopted and fit into V1 calculated land cost here, INCLUDE summation

    if VERSION == VERSION_TWO:
        land_cost_by_wwtp = toV1CostArray( treatment_train_data_v2, ["land_requirement_cost"])
        print('Land purchase cost for each wwtp employing a given TT =', land_cost_by_wwtp,
              ' ILS, a total for all the wwtps = ', sum(land_cost_by_wwtp))

    else:  # Initial method for calculating the land cost at each wwtp, total land cost for all outlets (coordinates)
        land_cost_by_wwtp = get_land_cost_by_wwtp(
            land_cost_in_wwtp_location, flow_wwtp, types_treatment_flow_based
        )
    ...
    ## TODO MD 4- ADD comments. This onsite sanitation is required as it provides solution for the population not served
    ...
    ## TODO MD 4: ACTION, comment
    ## MD- Even the benefit is to be made inactive##
    ...
    # Total costs
    #  TODO MD 4: ACTION- Write comment
    '''
    Gerald: 
    If method/version = Version 2,
    The land cost at the wwt plant is already factored for in each wwtp, 
    Therefore the sum(treatment_costs) already has sum(land_cost_by_wwtp),
    For Method 2, the cost of land should not be included in this calculation
    In other words, = sum(ww_pipe_inv_cost +sum(treatment_cost)
    '''
    if VERSION == VERSION_TWO:
        treatment_costs, land_cost_by_wwtp = normalizeV2ZeroFlows(ww_pipe_inv_costs, treatment_costs,land_cost_by_wwtp)
    total_costs_ww = sum(ww_pipe_inv_costs) + sum(treatment_costs) + sum(land_cost_by_wwtp)

    #total_costs_ww = sum(filter(None, ww_pipe_inv_costs)) + sum(filter(None, treatment_costs)) + sum(filter(None, land_cost_by_wwtp))
    ...
    # TODO MD 4: ACTION- Write comment
    '''
    The technology is part of technological selection basing on the flow/ Person equivalent in the catchment/town 
    '''
    ...
    display_selected_TT = types_treatment_flow_based
    if VERSION == VERSION_TWO:
        display_selected_TT = []
        for data in treatment_train_data_v2:
            if data is None:
                display_selected_TT.append(" ")
            else:
                display_selected_TT.append(data.get("display_train"," "))
    ...
    partial_results = {'Geographic coordinates (UTM)': list(coordinates),
                  ...
                  'Type of treatment of each wwtp': list(display_selected_TT),
...
def AOKP(coordinates):
    ...
    # In this cell the number of sub-catchments present in polygon.shp is
    ...
    if num_WWTP == num_subcatchments:
        ...
        #total_benefits, total_costs, coverage_catchment = Join_Calcul2(num_WWTP, coordinates)
