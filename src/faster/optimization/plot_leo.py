...
# CSV Files
csv_file_gen_pump_solutions = experiment_full_path + 'solutions' + experiment_id.lstrip("run") + '.csv'  # solutions
csv_file_gen_pump_results_total = experiment_full_path + 'results_total' + experiment_id.lstrip(
...
wwtp_labels = pd.read_csv(csv_file_gen_pump_solutions)

# Read CSV file with information about the WWTP configuration with no pumping
overall_info = pd.read_csv(csv_file_gen_pump_results_total)


# Create directed graph the results of genetic_pumping

def generate_graph_with_pumping(row_index, graph_nodes_data, graph_connections_data, df_overall_info, save_path=None):
    ...
    plt.title('Solution with pumps, ' + str(row_index) + ' in file results ' + experiment_id)
...
run_id = 4900
generate_graph_with_pumping(run_id, graph_data, wwtp_labels, overall_info,
                            save_path='experiment_full_path' + 'graph.png')  # Replace with the desired row index
...
# CSV Files
csv_file_no_pump_solutions = experiment_full_path + 'solutions' + experiment_id.lstrip("run") + '.csv'  # solutions
csv_file_no_pump_results_total = experiment_full_path + 'results_total' + experiment_id.lstrip(
    "run") + '.csv'  # results_total

# Read CSV file with graph information
graph_data = pd.read_csv(csv_file_graph_information) #(already read in the beginning of this file)

# Read CSV file with 'WWTP' labels no_pumping
wwtp_labels = pd.read_csv(csv_file_no_pump_solutions)

# Read CSV file with information about the WWTP configuration with no pumping
overall_info = pd.read_csv(csv_file_no_pump_results_total)


def generate_graph_no_pumping(selected_row, df_graph_data, df_wwtp_labels, df_overall_info):
    ...
    plt.title('Configuration of WWTPs - solution with no pumps' + str(selected_row))
    plt.xlabel('X-axis')
    plt.ylabel('Y-axis')
    plt.grid(True)
    plt.show()
...
generate_graph_no_pumping(63043, graph_data, wwtp_labels, overall_info)   #63043 is a good solution

# **************************************
# plot WWTP locations - This
...
# plt.title('Connection between candidate points according to natural flow in catchments')
# plt.show()
