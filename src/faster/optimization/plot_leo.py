...
csv_file_wwtp_results_total = experiment_full_path + 'results_total' + experiment_id.lstrip("run") + '.csv'     #results_total
...
# Read edges CSV file
edges_data = pd.read_csv(r'D:\IHE-Delft\2023\MSc 3-month theses\Hossein Tavakoligargari 2023\model\pipes.csv')  # Replace with your file path
# Read nodes CSV file
nodes_data = pd.read_csv(r'D:\IHE-Delft\2023\MSc 3-month theses\Hossein Tavakoligargari 2023\model\manholes.csv')  # Replace with your file path
...
#    plt.figure()  # Create a new figure for the graph
    ...
    for node in G.nodes:
        if node in wwtp_labels_dict:
            label = wwtp_labels_dict[selected_row]['connect_to_{}'.format(int(node))]  #label = wwtp_labels_dict[node][selected_row]['connect_to_{}'.format(int(node))]
            if label == 'NOTHING':
...
generate_graph(670, graph_data, wwtp_labels, overall_info)