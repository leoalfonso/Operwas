...
class OperwaFastGeneticPumping(OperwaFastBase):
    def __init__(self):
        ...
        # self.directions[3] = Problem.MAXIMIZE
    ...
    def evaluate(self, solution):
        ...
        benf_cost_ratio, coverage, results_total, results_nodes = Join_Calcul(
            data_dict, self._interpolators)

        assert benf_cost_ratio is not None
        assert coverage is not None

        solution.objectives[:] = (
            results_total["Benefits/costs"],
            results_total["Coverage (town)"],
            results_total["Number WWPS"],
            # results_total["Number activated"],
        )
